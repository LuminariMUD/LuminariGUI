...
class SystemTester:
    ...
    def _analyze_resource_usage(self, script_content, script_name, resource_usage):
        ...
        handler_registers = re.findall(
            r"registerAnonymousEventHandler\s*\(", script_content
        )
        handler_kills = re.findall(r"killAnonymousEventHandler\s*\(", script_content)

        resource_usage["handlers"]["registered"].extend(
            [(script_name, len(handler_registers))]
        )
    ...
    def run_tests(self):
        ...
        if resource_usage["potential_leaks"]:
            ...
            for leak in resource_usage["potential_leaks"]:
                print(
                    f"  {leak['script']}: {leak['type']} leak ({leak['created']} created, {leak['killed']} killed)"
                )
        else:
