...
# Import test modules
try:
    from test_coverage_reporting import CoverageReportingTester
...
class TestRunner:
    ...
    def run_all_tests(self, parallel=True, skip_optional=False):
        ...
        test_suites = [
            ...
            ("Coverage Reporting", CoverageReportingTester),
        ...
        for name, test_class in test_suites:
            if name in {"Embedded Lua Extraction", "Coverage Reporting"}:
                pass
    ...
    def run_single_test(self, test_name):
        """Run a single test suite."""
        test_map = {
            "coverage": ("Coverage Reporting", CoverageReportingTester),
...
def _print_verbose_configuration(args):
    ...
    print(
        f"  Missing optional tools: {'skip suites' if args.skip_optional else 'fail'}"
    )
...
def main():
    ...
    parser.add_argument(
        "--skip-optional",
        action="store_true",
        help="Skip tests with missing dependencies",
    )
    parser.add_argument(
        "--test",
