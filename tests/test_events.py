    ...
    def run_tests(self):
        ...
        if handlers:
            ...
            for handler in handlers:
                print(
                    f"  {handler['event']} -> {handler['handler']} ({handler['script']})"
                )
...
def main():
    ...
    parser.add_argument(
        "--list-handlers", action="store_true", help="List event handlers only"
    )
    ...
    if args.list_handlers:
        ...
        for handler in handlers:
            print(
                f"  {handler['event']} -> {handler['handler']} ({handler['script']}:{handler['line']})"
            )
        return
