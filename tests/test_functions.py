...
class LuaFunctionTester:
    ...
    def _is_testable(self, func_name, func_body):
        ...
        if any(
            ...
            for keyword in ["return", "local", "if", "for", "while"]
        ):
            return True
            ...
            for i, test_item in enumerate(test_case["tests"]):
                ...
                if success:
                    ...
                    if self._validate_result(
                        result, expected, expected_pattern, expected_table
                    ):
                        passed_tests += 1
                        ...
                        failed_tests += 1
                        expected_display = (
                            expected_table or expected or expected_pattern
                        )
                        print(
                            f"  ✗ Test {i + 1}: {test_item['input']} -> {result} (expected: {expected_display})"
                        )
                else:
                    failed_tests += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Lua function unit tests for LuminariGUI"
    )
