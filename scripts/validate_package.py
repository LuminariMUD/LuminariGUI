...
def validate_package(filename, include_lua_syntax=True):
    ...
    try:
        ...
        print(f"\n❌ XML Parse Error: {e}")
        print("   This usually means the XML is not well-formed.")
        print("   Check for:")
        print("   - Unclosed tags")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Validate LuminariGUI package (XML structure and Lua syntax)"
    )
    parser.add_argument(
        "filename",
        nargs="?",
    ...
    parser.add_argument(
        "--no-lua-syntax", action="store_true", help="Skip Lua syntax validation"
    )
    parser.add_argument(
    ...
    if args.quiet:
        ...
        with contextlib.redirect_stdout(f):
            success = validate_package(
                args.filename, include_lua_syntax=not args.no_lua_syntax
            )
